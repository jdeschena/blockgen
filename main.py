...
def _load_from_checkpoint(diffusion_model, config, tokenizer):
  # Released checkpoints are safetensors (EMA weights) + config.yaml, referenced
  # either by a local folder or an `hf:<repo>/<subfolder>` spec.
  if loading.is_safetensors_spec(config.eval.checkpoint_path):
    return loading.load_from_safetensors(
      diffusion_model, config, tokenizer)
...
def _sample_latency(diffusion_model, config, logger, tokenizer):
  """Compute 
        - Throughput with batch size config.loader.eval_batch_size
        - Compute Latency with batch size = 1"""
  logger.info('Starting to measure sampling latency and throughput.')
  model = diffusion_model(config, tokenizer).cuda()

  # Warmup (2)
  for bs in (1, config.loader.eval_batch_size):
    for _ in trange(2, desc='Warmup...'):
  ...
  # Throughput
  all_throughputs = []
  for _ in trange(config.sampler.num_sample_batches, 
                  desc='Computing throughput...'):
  ...
  if config.eval.results_json_path is None:
    ...
    save_fpath = os.path.join(save_dir, 
                    ...
                    f'num_steps={config.sampler.steps}.json')
  else:
...
def _latency_train(diffusion_model, config, logger, tokenizer):
  ...
  for _ in trange(cfg.num_timings + cfg.num_warmup, 
    ...
    latency = end - start
    b_eff = int(config.loader.batch_size)
    throughput = (b_eff * config.model.length) / latency
...
def _sample_gsm8k(diffusion_model, config, logger, tokenizer, fabric):
  ...
  # Bucket by prefix length so every batch has equal-length prefixes (required
  # by the block-ancestral sampler) and can therefore run at batch size > 1.
  import collections
  buckets = collections.defaultdict(list)
  for idx in local_indices:
    buckets[len(dataset[idx]['input_ids'])].append(idx)
  batches = [grp[i:i + batch_size]
  ...
  for batch_indices in tqdm(
      batches, desc=f'Rank {rank} sampling', disable=rank != 0):
    B = len(batch_indices)
...
def main(config):
  ...
  elif config.algo.name == 'blockgen-uniform':
    diffusion_model = algo.BlockGenUniform
  elif config.algo.name == 'blockgen-absorb':
    diffusion_model = algo.BlockGenAbsorb
  else:
    raise ValueError(
