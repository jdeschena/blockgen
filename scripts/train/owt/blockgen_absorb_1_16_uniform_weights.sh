...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/owt/blockgen_absorb_1_16_uniform_weights}"
NUM_NODES="${NUM_NODES:-1}"
DEVICES="${DEVICES:-16}"

cd "${REPO_ROOT}"

# Masked (absorbing) BlockGen with a 1+16 block-size mixture and uniform
# mixture weights (equal mass on every block size).
