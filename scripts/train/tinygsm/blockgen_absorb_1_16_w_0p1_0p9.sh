...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/tinygsm/blockgen_absorb_1_16_w_0p1_0p9}"
NUM_NODES="${NUM_NODES:-1}"
DEVICES="${DEVICES:-8}"

cd "${REPO_ROOT}"

# Masked (absorbing) BlockGen with a 1+16 block-size mixture (10% AR / 90% diffusion).
...
python -u -m main \
    ...
    "algo.block_weights=0.1 0.0 0.0 0.0 0.9" \
