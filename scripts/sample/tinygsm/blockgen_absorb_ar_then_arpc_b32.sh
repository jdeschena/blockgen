...
CKPT_PATH="${CKPT_PATH:-hf:jdeschena/blockgen/blockgen/multi_block_size/32/tinygsm/0.05_0.95/absorb}"
CACHE_DIR="${CACHE_DIR:-${REPO_ROOT}/data_cache}"
DATA_PATH="${DATA_PATH:-${REPO_ROOT}/data/gsm8k_test.json}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/eval_runs/tinygsm/blockgen_absorb_ar_then_arpc_b32}"
...
# (absorbing) multi-block 1+32 BlockGen model on GSM8K -- the headline TinyGSM
# method. Multi-block models share the KV cache between the AR and diffusion
# paths, so they use model.x0_causal=True; sampler.block_size must match the
# block size used during training.
