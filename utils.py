...
def validate_chunk_dim(hidden_size: int, chunk_dim: int) -> int:
  if chunk_dim <= 0:
    raise ValueError(
      f'Expected chunk_dim > 0, got {chunk_dim}')
  if hidden_size % chunk_dim != 0:
    raise ValueError(
      f'Expected hidden_size ({hidden_size}) to be divisible by '
      f'chunk_dim ({chunk_dim})')
  return hidden_size // chunk_dim


def to_chunk_view(x: torch.Tensor, chunk_dim: int) -> torch.Tensor:
  validate_chunk_dim(x.shape[-1], chunk_dim)
  return x.reshape(*x.shape[:-1], -1, chunk_dim)


def from_chunk_view(x: torch.Tensor) -> torch.Tensor:
  return x.reshape(*x.shape[:-2], -1)


def chunk_normalize(x: torch.Tensor, chunk_dim: int) -> torch.Tensor:
  x_chunks = to_chunk_view(x, chunk_dim)
  x_chunks = torch.nn.functional.normalize(x_chunks, dim=-1)
  return from_chunk_view(x_chunks)


def chunk_norms(x: torch.Tensor, chunk_dim: int) -> torch.Tensor:
  return torch.linalg.norm(to_chunk_view(x, chunk_dim), dim=-1)


def chunk_cosine(x: torch.Tensor, y: torch.Tensor,
                 chunk_dim: int, *, keepdim: bool = False):
  x_chunks = to_chunk_view(x, chunk_dim)
  y_chunks = to_chunk_view(y, chunk_dim)
  return (x_chunks * y_chunks).sum(dim=-1, keepdim=keepdim)


def chunk_slerp(clean: torch.Tensor, noisy: torch.Tensor,
                alpha_t: torch.Tensor, chunk_dim: int, eps: float):
  clean_chunks = to_chunk_view(clean, chunk_dim)
  noisy_chunks = to_chunk_view(noisy, chunk_dim)
  alpha_t = alpha_t.reshape(alpha_t.shape[0],
                            *([1] * (clean_chunks.ndim - 1)))
  cos_omega = (noisy_chunks * clean_chunks).sum(
    dim=-1, keepdim=True)
  ...
  out = coeff_clean * clean_chunks + coeff_noisy * noisy_chunks
  return from_chunk_view(out)


@torch.compile
def chunk_log_map(x: torch.Tensor, target: torch.Tensor,
                  chunk_dim: int, eps: float):
  x_chunks = to_chunk_view(x, chunk_dim)
  target_chunks = to_chunk_view(target, chunk_dim)
  cos_omega = (x_chunks * target_chunks).sum(dim=-1, keepdim=True)
  cos_omega = cos_omega.clamp(-1 + eps, 1 - eps)
  omega = torch.acos(cos_omega)
  scale = omega / omega.sin().clamp(min=eps)
  log_map = scale * (target_chunks - x_chunks * cos_omega)
  return from_chunk_view(log_map)


@torch.compile
def chunk_exp_map(x: torch.Tensor, delta: torch.Tensor,
                  chunk_dim: int, eps: float):
  x_chunks = to_chunk_view(x, chunk_dim)
  delta_chunks = to_chunk_view(delta, chunk_dim)
  delta_norm = delta_chunks.norm(dim=-1, keepdim=True).clamp(min=eps)
  x_new = (
    x_chunks * torch.cos(delta_norm)
    + (delta_chunks / delta_norm) * torch.sin(delta_norm)
  )
  return from_chunk_view(x_new)
